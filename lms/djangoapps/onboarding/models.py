...
from lms.djangoapps.onboarding import choices
from lms.djangoapps.onboarding.constants import (
    NOT_INTERESTED_KEY,
    NOT_INTERESTED_VAL,
...
class OrganizationPartner(models.Model):
    ...
    """
    organization = models.ForeignKey(Organization, related_name='organization_partners', on_delete=models.CASCADE)
...
class GranteeOptIn(models.Model):
    ...
    organization_partner = models.ForeignKey(OrganizationPartner, related_name='grantee_opt_in', on_delete=models.CASCADE)
    user = models.ForeignKey(User, related_name='grantee_opt_in', on_delete=models.CASCADE)
...
class OrganizationAdminHashKeys(TimeStampedModel):
    ...
    """
    organization = models.ForeignKey(Organization, related_name='suggested_admins', on_delete=models.CASCADE)
...
class UserExtendedProfile(TimeStampedModel):
    ...
    hear_about_philanthropyu = MultiSelectField(choices=choices.HEAR_ABOUT_PHILANTHROPY,
                                                         max_length=255,
                                                         max_choices=1,
                                                         blank=True)
    ...
    hubspot_contact_id = models.CharField(max_length=50, null=True)
...
class OrganizationMetric(TimeStampedModel):
    ...
    org = models.ForeignKey(Organization, related_name="organization_metrics", on_delete=models.CASCADE)
    user = models.ForeignKey(User, related_name="organization_metrics", on_delete=models.CASCADE)
...
class OrganizationMetricUpdatePrompt(models.Model):
    org = models.ForeignKey(Organization, related_name="organization_metrics_update_prompts", on_delete=models.CASCADE)
    responsible_user = models.ForeignKey(User, related_name="organization_metrics_update_prompts", on_delete=models.CASCADE)
